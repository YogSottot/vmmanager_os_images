        ...
        if [ "${MULTIIP}" = "true" ]; then \
                if [ "($IPv4ALIASES)" != "" ] && [ "($IPv4ALIASES)" != "()" ]; then
                        ...
                        for ipv4alias in ${IPv4ALIASES}; do
                                echo "" >> /etc/network/interfaces
                                echo "auto eth0:${ipnum}" >> /etc/network/interfaces
                                echo "allow-hotplug eth0:${ipnum}" >> /etc/network/interfaces
                                echo "iface eth0:${ipnum} inet static" >> /etc/network/interfaces
